...
/**
 * 마인크래프트 timerdisplay 사이드바의 순위 부분이 수정되었을 때 호출 (수정 패킷 수신한 틱 끝에 호출됨)
 *
 * 같은 틱에 이벤트가 여러 번 호출되어도 실제 호출은 가장 마지막 호출 한 번만 일어남
 *
 * invoker의 handle()은 항상 [io.github.oni0nfr1.dynamicrider.client.event.util.HandleResult.PASS]를 반환.
 *
 * 반환값:
 * - [io.github.oni0nfr1.dynamicrider.client.event.util.HandleResult.SUCCESS]: 더 이상 다른 리스너는 호출하지 않음
 * - [io.github.oni0nfr1.dynamicrider.client.event.util.HandleResult.PASS]: 다음 리스너로 넘김
 *
 * **주의**
 * sidebar 객체는 mutable인데, 내부 값을 수정하면 이후 핸들러에 부작용이 일어날 수 있음
 */
fun interface RiderRankingUpdateCallback {
